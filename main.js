...
(function initAdvancedInteractions() {
  ...
  if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
    ...
    /* ─ Horizontal story scroll — How It Works ─ */
    const storyRegion = document.getElementById('story-scroll-region');
    const storyTrack  = document.getElementById('story-track');
    if (storyRegion && storyTrack) {
      const progFill = document.querySelector('.story-progress-fill');
      const dots     = document.querySelectorAll('.story-dot');
      const panels   = document.querySelectorAll('.story-panel');
      const counter  = document.getElementById('storyCurrentPanel');
      ...
      ScrollTrigger.create({
        ...
        scrub: 1,
        onUpdate(self) {
          const travel = storyTrack.scrollWidth - window.innerWidth;
          gsap.set(storyTrack, { x: -(self.progress * travel) });
          ...
          if (counter) counter.textContent = nums[active];
        }
      });
    }
